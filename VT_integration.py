...
class VirusTotal(AppIntegration):
	...
	def scan_and_report(self, resource):
		"""Takes in RESOURCE, which is the url of the file in question
		and tries to retrieve that url's report. If the report does
		not exist, it will scan the url and wait for a report to be
		generated before returning the response dictionary."""
